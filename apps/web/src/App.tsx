...
type ViewName = "dashboard" | "feed" | "reports";
...
function App() {
  ...
  const [reportsDate, setReportsDate] = useState<string | null>(null);
  ...
  // Load the day's reports when the reports tab opens. The read endpoint is
  // public, but reports only exist for registered symbols, so gate on sign-in.
  useEffect(() => {
    if (view !== "reports" || !authUser) return;
    let cancelled = false;
    ...
    api
      ...
      .then((data) => {
        ...
        setReportsDate(data.report_date);
      })
      .catch(() => {
        if (cancelled) return;
        setReports([]);
...
/**
 * Daily report tab: one card per registered symbol with the morning briefing
 * and a buy/hold/sell opinion. Generated by the worker on weekday mornings;
 * the read endpoint returns the latest published date's reports.
 */
  ...
  if (loading) {
    return (
      <div className="reports-view">
        <div className="reports-empty">리포트를 불러오는 중…</div>
      </div>
    );
  }
  if (reports.length === 0) {
    return (
      <div className="reports-view">
        <div className="reports-empty">
          <FileText size={36} />
          <div>아직 생성된 데일리 리포트가 없습니다.</div>
          <div className="muted">
            평일 오전 8시 30분 이후 등록 종목의 리포트가 준비됩니다.
          </div>
        </div>
      </div>
    );
    ...
    <div className="reports-view">
      {reportDate && (
        <div className="reports-head">
          <strong>{formatTradeDate(reportDate)}</strong> 데일리 리포트
          <span className="muted"> · {reports.length}개 종목</span>
        </div>
      )}
...
function ReportCard({ report }: { report: DailyReportItem }) {
  const change = report.change_pct;
  return (
    <article className="report-card">
      <header className="report-card-head">
        <div className="report-card-title">
          <MktChip market={report.symbol_market} />
          <span className="report-card-name">{report.symbol_name}</span>
          <span className="report-card-code mono">{report.symbol_code}</span>
        </div>
        <span className={`rec-badge rec-badge--${report.recommendation}`}>
          {recommendationLabel(report.recommendation)}
        </span>
      </header>
      {report.prev_close !== null && (
        <div className="report-card-price">
          <span className="mono">{report.prev_close.toLocaleString()}원</span>
          {change !== null && (
            <span
              className={`report-card-change ${change >= 0 ? "is-pos" : "is-neg"}`}
            >
          ...
          {report.prev_trade_date && (
            <span className="muted">
              {" "}
              · {formatTradeDate(report.prev_trade_date)} 종가
            </span>
          )}
        </div>
      )}
      ...
      {report.rationale && (
        <p className="report-card-rationale muted">{report.rationale}</p>
      )}
    </article>
  );
}
